...
class Photos18Spider(scrapy.Spider):
    ...
    def parse(self, response):
        category_names = response.xpath('//*[@id="w3"]/li/a/p/text()').extract()
        category_links = response.xpath('//*[@id="w3"]/li/a/@href').extract()
        for i in range(0,17):
            self.logger.info(f"{category_names[i]} - {self.start_urls[0] + category_links[i]}")
            yield scrapy.Request(self.start_urls[0] + category_links[i], callback=self.parse_1)


    def parse_1(self, response):
        collections_list = response.xpath('//*[@id="videos"]/div/div[2]/a/text()')
        collections_link_list = response.xpath('//*[@id="videos"]/div/div[2]/a/@href')
        for i in range(100):
            collection_name = collections_list[i].extract()
            collection_link = self.start_urls[0] + collections_link_list[i].extract()
            self.logger.info(f"{collection_name} - {collection_link}")
            yield scrapy.Request(collection_link, callback=self.parse_2)

        next_page = response.xpath('//*[@id="w0"]/ul/li[7]/a/@href').extract_first()
    ...
    def parse_2(self, response):
        item = NiceGirlsItem()
        category_name = response.xpath('//*[@id="w4"]/li[2]/a/text()').extract_first()
        page_name = response.css('title::text').get()
        image_links = response.xpath('//*[@id="content"]/div/a/img/@data-src').extract()
